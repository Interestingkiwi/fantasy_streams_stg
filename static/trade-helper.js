(async function() {
    ...
    async function init() {
        ...
        // --- 1. Inject PP Modal HTML (Fix for Missing Modal) ---
        const ppModalHTML = `
        ...
        document.body.addEventListener('click', (e) => {
            ...
            // C. Handle Simulate Button Click
    ...
    function findTradeMatches(matrix, totalTeams) {
        ...
        // Note: SV/SA kept for internal sorting
        let matches = [];
    ...
    function renderComparePage() {
        ...
        // IMPORTANT: Update button state after rendering (since checkboxes are new)
        updateSimulateButtonState();
    }

    // --- Button State Logic (Fix for Clickability) ---
    function updateSimulateButtonState() {
        ...
        // Rule 1: Must have at least 1 player from each side
        ...
        // Rule 2: All opponent players must be from the SAME team
    ...
    // --- Helper: Setup Sorting (Unchanged) ---
    ...
    function removeCategory(type, category) {
        if (type === 'from') activeFromCats = activeFromCats.filter(c => c !== category); else activeToCats = activeToCats.filter(c => c !== category);
        const container = type === 'from' ? document.getElementById('trade-from-tags') : document.getElementById('trade-to-tags');
